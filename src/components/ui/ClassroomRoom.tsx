...
function Desk({
  ...
  return (
    ...
    >
      {/* Glass card */}
      <div
        className={`w-16 h-16 md:w-[76px] md:h-[76px] rounded-2xl relative flex items-center justify-center overflow-hidden transition-all duration-300 ${isOwn
      ...
      >
        {empty ? (
          <span className="text-2xl text-[#BAE6FD] group-hover:text-[#06B6D4] transition-colors"><Icon icon="tdesign:user-add-filled" width={40} /></span>
        ) : (
          <>
            {participant!.studentAvatar ? (
              ...
              <div className={`w-full h-full flex items-center justify-center font-bold text-xl text-white ${isOwn ? 'bg-gradient-to-br from-[#06B6D4] to-[#0369A1]' : 'bg-gradient-to-br from-cyan-400 to-blue-500'
                }`}>
                {initial}
              </div>
            )}
      ...
      {/* Name label */}
      <p className={`text-[9px] font-semibold text-center leading-tight w-20 break-words line-clamp-2 ${empty ? 'text-[#94A3B8]' : isOwn ? 'text-[#06B6D4]' : 'text-[#475569]'
        }`}>
          ...
          >
            ...
            <div className="relative z-10 px-6 py-5 flex flex-col items-center gap-3">
              ...
              {editingAnnouncement && isTeacher ? (
                ...
                <>
                  {announcement ? (
                    <p className="flex items-center gap-2 text-center text-white/90 text-base md:text-lg font-medium max-w-xl"
                      style={{ fontFamily: '"Segoe UI", sans-serif', textShadow: '0 0 20px rgba(255,255,255,0.3)' }}>
                      <Icon icon="ant-design:notification-filled" width={22} /> {announcement}
                    </p>
                  ) : (
                  ...
                  {isTeacher && (
                    <button onClick={() => { setDraftAnnouncement(announcement); setEditingAnnouncement(true); }}
                      ...
                      <Icon icon="ant-design:notification-outlined" width={12} /> {announcement ? 'Sửa thông báo' : 'Viết lên bảng'}
                    </button>
                  )}
                </>
              )}
            ...
            >
              ...
              {isTeacher && (
                <div className="mt-3 flex items-center gap-2">
                  ...
                  >
                    <Icon icon="rivet-icons:question-mark-solid" width={16} /> Tạo câu hỏi
                  </button>
                  {savedQuestions.length > 0 && !liveKitQuizActive && (!classroom.activeQuiz || classroom.activeQuiz.isFinished) && (
                    ...
                    >
                      <Icon icon="material-symbols:not-started-rounded" width={16} /> Bắt đầu ({savedQuestions.length} câu)
                    </button>
                  )}
                </div>
              )}
