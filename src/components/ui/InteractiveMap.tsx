...
import {
  MapContainer, TileLayer, Marker, CircleMarker, Polyline, Polygon, Tooltip, useMap, ImageOverlay
} from 'react-leaflet';
...
const TILES: Record<MapMode, { url: string; attr: string; overlay?: string }> = {
  // Ảnh vệ tinh ESRI (màu xanh cây, xanh biển tự nhiên) + viền biên giới từ CartoDB
  political: {
  ...
  vietnam:   { url: 'https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',                       attr: '© OpenStreetMap' },
};
...
const LEGENDS: Record<MapMode, { color?: string; icon: string; label: string }[]> = {
  ...
  vietnam: [
    { icon: '🔵', label: '8 Vùng kinh tế - xã hội' },
...
// ── Vietnam markers ───────────────────────────────────────────────────────────
function VietnamMarkers({ features, onSelect }: { features: GeoItem[]; onSelect: (item: GeoItem) => void }) {
  const regions = features.filter(f => f.subCategory === 'vnRegion');
  const cities  = features.filter(f => f.subCategory === 'vnCity');
  return (
    <>
      {regions.map(r => (
        <CircleMarker key={r.id} center={[r.lat, r.lng]} radius={16}
          pathOptions={{ color: 'white', fillColor: r.color || '#3b82f6', fillOpacity: 0.8, weight: 2 }}
          eventHandlers={{ click: () => onSelect(r) }}>
          <Tooltip direction="top" offset={[0,-10]} opacity={1}>
            <span className="font-bold text-xs">{r.name}</span>
          </Tooltip>
      ...
      {cities.map(c => (
        <Marker key={c.id} position={[c.lat, c.lng]} icon={emojiIcon(c.emoji || '🏙️', 20)}
          eventHandlers={{ click: () => onSelect(c) }}>
          <Tooltip direction="top" offset={[0,-8]} opacity={1}>
            <span className="font-bold text-xs">{c.name}</span>
          </Tooltip>
...
export default function InteractiveMap({ is3D, onToggle3D, mode, onModeChange }: InteractiveMapProps) {
  ...
  const [geoFeatures, setGeoFeatures] = useState<GeoItem[]>([]); // Data dùng chung cho các tab (mountain, climate...)
    ...
    >
      ...
      <div className="absolute inset-0">
        ...
        >
          ...
          {mode === 'political' && <PoliticalMarkers countries={countries} onSelect={handleSelectGeo} />}
          ...
          {mode === 'vietnam'   && <VietnamMarkers   features={geoFeatures} onSelect={handleSelectGeo} />}
        </MapContainer>
      </div>
      ...
      <div className={`absolute top-[76px] right-4 w-[290px] max-w-[calc(100vw-32px)] bg-white/85 backdrop-blur-2xl border border-white rounded-[24px] shadow-[0_20px_40px_rgba(8,47,73,0.12)] p-5 z-[999] transition-all duration-400 ${panelView !== 'none' ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8 pointer-events-none'}`}>
        ...
        {panelView === 'legend' && (
          <>
            ...
            <p className="text-[10px] font-bold text-cyan-600 uppercase tracking-wider mb-2.5">
      ...
      <div className="absolute bottom-5 left-1/2 -translate-x-1/2 z-[1000] flex items-center gap-1.5 p-1.5 bg-white/65 backdrop-blur-2xl border border-white rounded-[28px] shadow-[0_10px_30px_rgba(8,47,73,0.15)] overflow-x-auto max-w-[calc(100vw-24px)]">
        {MODES.map(m => (
          <button
            ...
            className={`flex flex-col items-center justify-center min-w-[68px] h-[58px] rounded-[20px] font-bold text-xs transition-all duration-300 whitespace-nowrap px-2 ${
              mode === m.id
