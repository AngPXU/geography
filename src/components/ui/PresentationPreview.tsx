...
function GroupActivityTimer({ duration }: { duration: string }) {
  ...
  return (
    <div className={`flex flex-col items-center gap-3 py-4 px-5 rounded-2xl border-2 transition-all duration-500 ${done ? 'bg-emerald-50 border-emerald-400 shadow-[0_0_24px_rgba(34,197,94,0.25)]' :
...
export function PresentationPreview({ blocks, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    // Cache elements once per render to avoid heavy DOM queries on every scroll tick
    const mapActionElements = Array.from(document.querySelectorAll('.preview-media-action'));
    let currentActiveEl: Element | null = null;
    let ticking = false;
    ...
    const handleScroll = () => {
      if (!ticking) {
        window.requestAnimationFrame(() => {
          ...
          // Loop backwards to find the FIRST element whose center is above the viewport center.
          // This avoids checking elements higher up once we found the active one.
          for (let i = mapActionElements.length - 1; i >= 0; i--) {
          ...
          // Only trigger state updates if the active element actually changed!
          if (activeEl === currentActiveEl) {
            ticking = false;
            return;
          }
          ...
          if (type === 'mapAction') {
            ...
            setActiveMediaBlock({ type: 'openQuestion', questionImage: imgUrl });
          } else if (type === 'gallery') {
            const urlsAttr = activeEl.getAttribute('data-imageurls') || '[]';
            let parsedUrls: string[] = [];
            try { parsedUrls = JSON.parse(urlsAttr); } catch (e) { }
            setActiveMediaBlock({ type: 'gallery', imageUrls: parsedUrls });
          }
          ticking = false;
        });
        ticking = true;
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[99999] flex flex-col lg:flex-row bg-[#E0F2FE] animate-in fade-in overflow-hidden">
      ...
      <div className="left-panel-preview flex flex-col relative z-20 bg-white/70 backdrop-blur-2xl border-b lg:border-b-0 lg:border-r border-white/80 shadow-[20px_0_50px_rgba(14,165,233,0.1)] w-full lg:w-1/2 lg:min-w-[500px] overflow-hidden">
        ...
        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto overflow-x-hidden pt-16 lg:pt-32 pb-16 lg:pb-64 w-[95%] mx-auto space-y-6 lg:space-y-10 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          {blocks.map((block) => {
            ...
            if (block.type === 'objectives') {
              ...
              return (
                <div key={block.id} className="relative z-10 bg-gradient-to-br from-orange-50 to-amber-50 backdrop-blur-xl p-8 rounded-3xl border border-orange-200 shadow-[0_10px_30px_rgba(245,158,11,0.15)] my-8">
            ...
            if (block.type === 'imageScenario') {
              return (
                <div
                  key={block.id}
                  id={`block-${block.id}`}
                  className="preview-media-action relative z-10 bg-white border-2 border-emerald-400 rounded-[32px] p-8 shadow-md my-12"
            ...
            if (block.type === 'funFact') {
              return (
                <div key={block.id} className="relative z-10 bg-gradient-to-br from-[#E0F2FE]/90 to-[#DCFCE7]/90 backdrop-blur-xl p-8 rounded-3xl border border-white shadow-[0_10px_30px_rgba(14,165,233,0.1)]">
            ...
            if (block.type === 'quiz') {
              ...
              return (
                <div key={block.id} className="relative z-10 bg-white/80 backdrop-blur-xl rounded-3xl border border-orange-200 shadow-[0_10px_30px_rgba(249,115,22,0.12)] overflow-hidden">
                  {/* Progress bar + header */}
                  <div className="px-6 py-3 flex items-center justify-between" style={{ background: 'linear-gradient(to right, #ea580c, #f97316)' }}>
                      ...
                      <div className="flex items-center gap-1.5">
                        {questions.map((_, i) => (
                          <button
                            ...
                            className={`transition-all duration-200 rounded-full ${i === currentQIdx ? 'w-5 h-5 bg-white text-orange-600 text-[10px] font-black flex items-center justify-center shadow' :
                        ...
                        return (
                          <button key={idx} className={cls} onClick={() => !hasAnswered && setQuizAnswers(prev => ({ ...prev, [answerKey]: idx }))}>
                            <span className="flex items-center gap-3">
                              <span className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-black shrink-0 transition-all ${!hasAnswered ? 'bg-orange-100 text-orange-600' :
            ...
            if (block.type === 'video') {
              ...
              return (
                <div key={block.id} className="relative z-10 bg-gradient-to-br from-red-50 to-pink-50 backdrop-blur-xl p-6 rounded-3xl border border-red-200 shadow-[0_10px_30px_rgba(239,68,68,0.1)] my-6">
            ...
            if (block.type === 'chart') {
              ...
              return (
                <div key={block.id} className="relative z-10 bg-white/95 backdrop-blur-xl p-6 rounded-3xl border border-indigo-100 shadow-[0_10px_30px_rgba(99,102,241,0.08)] my-6">
                  {block.title && (
            ...
            if (block.type === 'diagram') {
              return (
                <div key={block.id} className="relative z-10 bg-white/90 backdrop-blur-xl p-6 rounded-3xl border border-teal-200 shadow-[0_10px_30px_rgba(20,184,166,0.1)] my-6">
            ...
            if (block.type === 'compare') {
              ...
              return (
                <div key={block.id} className="relative z-10 my-6 space-y-6">
                  {block.title && (
            ...
            if (block.type === 'callout') {
              ...
              return (
                <div key={block.id} className={`relative z-10 ${style.bg} border-l-4 ${style.border} rounded-r-2xl p-6 my-6 shadow-sm`}>
            ...
            if (block.type === 'timeline') {
              return (
                <div key={block.id} className="relative z-10 my-6">
            ...
            if (block.type === 'groupActivity') {
              return (
                <div key={block.id} className="relative z-10 bg-gradient-to-br from-lime-50 to-green-50 backdrop-blur-xl p-6 rounded-3xl border-2 border-lime-300 shadow-[0_10px_30px_rgba(132,204,22,0.1)] my-6">
            ...
            if (block.type === 'openQuestion') {
              ...
              return (
                <div
                  key={block.id}
                  className="preview-media-action relative z-10 bg-white/80 backdrop-blur-xl rounded-3xl border border-cyan-200 shadow-[0_10px_30px_rgba(6,182,212,0.12)] overflow-hidden"
                    ...
                    {block.openAnswer && (
                      <div>
                        <button
                          ...
                          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-black transition-all ${shown
            ...
            if (block.type === 'fillBlank') {
              ...
              return (
                <div key={block.id} className="relative z-10 bg-gradient-to-br from-yellow-50 to-amber-50 backdrop-blur-xl p-6 rounded-3xl border-2 border-amber-300 shadow-[0_10px_30px_rgba(245,158,11,0.1)] my-6">
            ...
            if (block.type === 'quote') {
              return (
                <div key={block.id} className="relative z-10 bg-gradient-to-br from-stone-50 to-zinc-50 border-l-4 border-stone-400 rounded-r-2xl p-8 my-8">
            ...
            if (block.type === 'gallery') {
              ...
              if (isPanel) {
                ...
                return (
                  <div
                    key={block.id}
                    className="preview-media-action relative z-10 my-8"
                    data-type="gallery"
            ...
            if (block.type === 'summary') {
              ...
              return (
                <div key={block.id} className="relative z-10 my-8">
      ...
      <div className="right-panel-preview w-full flex flex-col p-2 lg:p-0 relative shrink-0 z-10 bg-white/40 lg:bg-[#0a1628] backdrop-blur-3xl lg:backdrop-blur-none border-t lg:border-0 border-white/80 rounded-t-[24px] lg:rounded-none">
        ...
        <div className="flex-1 flex flex-col items-center justify-center relative rounded-xl lg:rounded-none overflow-hidden border-4 lg:border-0 border-white/80 lg:border-transparent shadow-[0_20px_50px_rgba(14,165,233,0.2)] lg:shadow-none bg-[#082F49] pointer-events-none w-full h-full">
          <div className="pointer-events-auto w-full h-full relative">
            ...
            <div className={`absolute inset-0 bg-transparent transition-all duration-500 ease-in-out ${activeMediaBlock?.type === 'imageScenario' ||
