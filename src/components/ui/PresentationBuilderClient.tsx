...
export function PresentationBuilderClient({ user, presentationId, onBack }: Props) {
  ...
  const getBlockIcon = (type: string) => {
    switch (type) {
      ...
      case 'text': return '📝';
      case 'imageScenario': return '🖼️';
      case 'objectives': return '🎯';
      case 'funFact': return '💡';
      case 'mapAction': return '🌍';
      case 'quiz': return '❓';
  ...
  const getBlockExcerpt = (block: StoryBlock) => {
    ...
    if (block.type === 'objectives') return block.content?.substring(0, 50) + '...' || '(Chưa có mục tiêu)';
    if (block.type === 'funFact') return block.title || '(Chưa có tiêu đề)';
  ...
  const renderBlockEditor = (block: StoryBlock) => {
    ...
    return (
      <div
        ...
        className={`group relative rounded-2xl transition-all duration-300 border-2 cursor-pointer
      ...
      >
        ...
        <div className={`absolute -right-3 -top-3 flex flex-col gap-1 bg-white p-1 rounded-xl shadow-lg border border-slate-100 transition-opacity z-20 ${isActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
          <button onClick={toggleCollapse} className="w-8 h-8 rounded-lg text-slate-500 hover:bg-slate-100 flex items-center justify-center font-bold text-xs" title={isCollapsed ? "Mở rộng" : "Thu gọn"}>
            {isCollapsed ? 'Mở' : 'Thu'}
          </button>
          ...
          <>
            ...
            {block.type === 'funFact' && (
              <div className="bg-gradient-to-br from-blue-50 to-cyan-50 border border-cyan-200 rounded-2xl p-6 relative overflow-hidden">
                ...
                <input
                  ...
                  className="w-full bg-transparent outline-none font-bold text-cyan-700 mb-2 text-lg"
                  placeholder="Tiêu đề hộp nổi bật..."
                />
                <div className="bg-white/80 rounded-xl overflow-hidden border border-cyan-100 mt-2">
                  <RichTextEditor
                    value={block.content || ''}
                    onChange={(html) => updateBlock(block.id, { content: html })}
            ...
            {block.type === 'imageScenario' && (
              <div className="bg-white border-2 border-emerald-400 rounded-2xl p-6 relative overflow-hidden shadow-sm">
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl overflow-hidden focus-within:border-emerald-400">
                  <RichTextEditor
                    value={block.content || ''}
                    onChange={(html) => updateBlock(block.id, { content: html })}
            ...
            {block.type === 'objectives' && (
              <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-orange-200 rounded-2xl p-6 relative overflow-hidden">
                ...
                <div className="bg-white/80 rounded-xl overflow-hidden border border-orange-100">
                  <RichTextEditor
                    value={block.content || ''}
                    onChange={(html) => updateBlock(block.id, { content: html })}
